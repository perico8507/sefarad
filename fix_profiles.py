...
def get_tokens(s):
    ...
    return set(re.split(r'[^a-zA-Z0-9]+', remove_accents(s).lower()))
...
for txt_path in txt_files:
    ...
    # if no strong overlap, maybe substring without special chars
    if best_score == 0:
    ...
    matches[txt_path] = best_match

print("Matches found:", sum(1 for v in matches.values() if v is not None), "out of", len(txt_files))
for txt, img in matches.items():
    if not img:
        print("NO MATCH FOR:", txt)
